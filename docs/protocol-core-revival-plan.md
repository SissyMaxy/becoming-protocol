...
- ⏳ **Stage 7 — blocked on 5 (domain modules) + 6.** Thin the 11,944-line god-module (`chat-action.ts`) to an entrypoint by migrating its ~120 `build*Ctx` builders + ~25 analyzers into modules; delete handler-v2 (47 files, 24 live importers); remove the `PROTOCOL_CORE_FLOWS` flags + the legacy inline reward/note fallbacks (only after the flags are live-verified permanently ON). Cannot start until the behaviors those deletions remove exist with parity in protocol-core. **Largest remaining body of work.**
