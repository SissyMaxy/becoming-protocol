...
- 🔶 **Stage 7 — god-module thinning STARTED (major dent); handler-v2 deletion remains.**
  - ✅ **94 context builders extracted** (commit 2529674): the self-contained `build*Ctx` functions moved VERBATIM into `api/handler/_lib/handler-context-builders.ts` (~5.9k lines) via the TS parser for exact spans. **chat-action.ts 11,969 → 5,586 lines (−53%).** Shared caches/consts/helpers stay in chat-action (exported + imported back); call-time-only cross-refs → load-safe cycle. Behavior-identical; FULL suite (1644) green. pattern-lint + voice-gate baselines refreshed (pure relocation, precedent 6d397ab).
  - ⏳ **~25 leaf functions remain in chat-action** (~2.1k lines): the analyzers/gates (runArousalDetection, runPronounGate, semanticSlipDetect, checkBehavioralTriggers…) and the side-effect executors (logDirectiveOutcome, executeDeviceCommand, searchContent, and `handleForceFeminizationDirective` ~860L — force-fem, migrate LAST/never soften). These are entangled with `handleChat`, the persist injection path, and the context-builders (which import semanticMemorySearch + calculateBiometricDeviceIntensity), so extraction now means real cross-module rewiring — do it with the full suite + a smoke per batch, force-fem last.
  - ⏳ **handler-v2 deletion (47 files, 24 live importers) + flag removal** still need domain-module parity + live app-run verification (see Stage 6). Remove `PROTOCOL_CORE_FLOWS` + the legacy inline reward/note fallbacks only after the flags are live-verified permanently ON.
