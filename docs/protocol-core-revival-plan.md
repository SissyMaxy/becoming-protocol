...
- 🔄 **Stage 5 IN PROGRESS.** Pattern established; behaviors migrating one flow at a time, each flag-gated + parity-tested.
  ...
  - ⏳ **5b+ REMAIN — the directive loop.** Everything left in `persistTurnSideEffects` is ONE interleaved for-loop (directive-log insert + `logDirectiveOutcome` learning hook + 18 execution branches + the 6 streaming-only branches + force-femme LAST). No further cleanly-separable standalone block remains; the directive-loop migration is a single larger surgery. The 39 characterization tests are the parity net. **Do this with live app-run verification — and land Stage 7's cleanup close behind it, since a flagged copy duplicates the loop that Stage 2 just de-duplicated (drift risk while both live).**
- ⏳ **Stages 6, 7 REMAIN — structurally different, riskier.** 6 = make `useHandlerPrescription` real + reconcile two mode machines + repoint ~25 handler-v2 importers (frontend). 7 = thin out the god-module, delete handler-v2, remove flags. Both need app-run verification.
- **Verification gap (important):** Stages 4 + 5a are unit/typecheck/gate-verified and committed with flags OFF, so production is untouched. Flipping any flag ON still needs a live app-run (real device + streaming against prod Supabase) — that step is the user's environment, not reproducible in the build sandbox.
