-- 428 — Fix Postgres word-boundary mismatch in mommy_voice_cleanup.
--
-- Bug: PostgreSQL regex engine treats `\b` differently from PCRE/JS —
-- specifically, `\b` does NOT anchor reliably immediately after
-- `\s+saying` (it returned the input unchanged in live testing,
-- 2026-05-15). The Postgres-native word-boundary metas are `\m`
-- (start) and `\M` (end). Several 269/425 rules used `\b` and silently
-- failed, leaving clinical text un-cleaned. Symptoms:
--
...
--
-- After 425 the "overdue by N hours" rule fired (M-anchored, fine) and
-- "Open the camera" fired (M-anchored, fine), but "Record yourself
-- saying" / "Submit it now." / "you'll be ready for it" / "take the
-- shot" all had trailing `\b` and stayed in the output. This migration
-- re-defines mommy_voice_cleanup with every word-boundary anchor
-- normalized to `\m` / `\M` and adds a catch-all `record yourself` rule
-- so terse phrasings without `saying` still get scrubbed.
--
-- Also adds a wider live backfill that re-cleans:
--   - assistant turns in handler_messages (now that 427 added the trigger)
--   - pending handler_outreach_queue
--   - active handler_decrees
-- for any mommy-persona user where the cleaned text differs from current.
...
BEGIN
  ...
  -- 425/428 — chat leaks 2026-05-14/15. \b -> \M (Postgres-correct).
...
-- Re-clean now that all rules fire. Three surfaces.
