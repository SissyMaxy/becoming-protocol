-- PART 1: Tables, indexes, RLS policies (run first)
-- Migrations 123-134 DDL
...
DROP POLICY IF EXISTS "Users can read own whoop tokens" ON whoop_tokens;
CREATE POLICY "Users can read own whoop tokens" ON whoop_tokens
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can update own whoop tokens" ON whoop_tokens;
...
DROP POLICY IF EXISTS "Users can read own whoop metrics" ON whoop_metrics;
...
DROP POLICY IF EXISTS "Users can read own handler params" ON handler_parameters;
CREATE POLICY "Users can read own handler params" ON handler_parameters
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can update own handler params" ON handler_parameters;
...
DROP POLICY IF EXISTS "Users can read own generated tasks" ON generated_tasks;
...
DROP POLICY IF EXISTS "Users can read own resistance events" ON resistance_events;
...
DROP POLICY IF EXISTS "Users can read own ab tests" ON ab_tests;
...
DROP POLICY IF EXISTS "Users can read own predictions" ON state_predictions;
...
DROP POLICY IF EXISTS "Users can read own novelty events" ON novelty_events;
...
DROP POLICY IF EXISTS "Users own comfort map" ON gina_comfort_map;
...
DROP POLICY IF EXISTS "Users own timing" ON gina_timing_data;
...
DROP POLICY IF EXISTS "Users own signals" ON gina_disclosure_signals;
...
DROP POLICY IF EXISTS "Users own vault" ON content_vault;
...
DROP POLICY IF EXISTS "Users own posts" ON content_posts;
...
DROP POLICY IF EXISTS "Users own interactions" ON fan_interactions;
...
DROP POLICY IF EXISTS "Users own highlights" ON cam_highlights;
...
DROP POLICY IF EXISTS "Users own conversations" ON handler_conversations;
...
DROP POLICY IF EXISTS "Users own messages" ON handler_messages;
...
DROP POLICY IF EXISTS "Users own outreach" ON handler_outreach;
...
DROP POLICY IF EXISTS "Users own conditioning protocols" ON conditioning_protocols;
...
DROP POLICY IF EXISTS "Users own conditioning sessions" ON conditioning_sessions;
...
DROP POLICY IF EXISTS "Users own conditioned triggers" ON conditioned_triggers;
...
DROP POLICY IF EXISTS "Users own hrt pipeline" ON hrt_pipeline;
...
DROP POLICY IF EXISTS "Users own hrt changes" ON hrt_changes;
...
DROP POLICY IF EXISTS "Users own hrt doses" ON hrt_doses;
...
DROP POLICY IF EXISTS "Users own social web" ON social_web;
...
DROP POLICY IF EXISTS "Users own collab pipeline" ON collaboration_pipeline;
...
DROP POLICY IF EXISTS "Users own shame architecture" ON shame_architecture;
...
DROP POLICY IF EXISTS "Users own shame exposures" ON shame_exposures;
...
DROP POLICY IF EXISTS "Users own revenue" ON revenue_tracking;
...
DROP POLICY IF EXISTS "Users own crossover" ON crossover_tracking;
...
DROP POLICY IF EXISTS "Users own masculine contexts" ON masculine_contexts;
...
DROP POLICY IF EXISTS "Users own language monitoring" ON language_monitoring;
...
DROP POLICY IF EXISTS "Users read own ai_generated_content" ON ai_generated_content;
CREATE POLICY "Users read own ai_generated_content" ON ai_generated_content
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users read own engagement_targets" ON engagement_targets;
CREATE POLICY "Users read own engagement_targets" ON engagement_targets
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users read own revenue_content_calendar" ON revenue_content_calendar;
CREATE POLICY "Users read own revenue_content_calendar" ON revenue_content_calendar
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users read own paid_conversations" ON paid_conversations;
CREATE POLICY "Users read own paid_conversations" ON paid_conversations
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users read own gfe_subscribers" ON gfe_subscribers;
CREATE POLICY "Users read own gfe_subscribers" ON gfe_subscribers
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users read own affiliate_links" ON affiliate_links;
CREATE POLICY "Users read own affiliate_links" ON affiliate_links
  FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users read own revenue_decisions" ON revenue_decisions;
...
DROP POLICY IF EXISTS "Service manages ai_generated_content" ON ai_generated_content;
CREATE POLICY "Service manages ai_generated_content" ON ai_generated_content
  FOR ALL USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Service manages engagement_targets" ON engagement_targets;
CREATE POLICY "Service manages engagement_targets" ON engagement_targets
  FOR ALL USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Service manages revenue_content_calendar" ON revenue_content_calendar;
CREATE POLICY "Service manages revenue_content_calendar" ON revenue_content_calendar
  FOR ALL USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Service manages paid_conversations" ON paid_conversations;
CREATE POLICY "Service manages paid_conversations" ON paid_conversations
  FOR ALL USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Service manages gfe_subscribers" ON gfe_subscribers;
CREATE POLICY "Service manages gfe_subscribers" ON gfe_subscribers
  FOR ALL USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Service manages affiliate_links" ON affiliate_links;
CREATE POLICY "Service manages affiliate_links" ON affiliate_links
  FOR ALL USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Service manages revenue_decisions" ON revenue_decisions;
...
-- [cron job moved to separate file]
...
ALTER TABLE handler_memory DROP CONSTRAINT IF EXISTS importance_5_no_decay;
ALTER TABLE handler_memory ADD CONSTRAINT importance_5_no_decay CHECK (importance < 5 OR decay_rate = 0);

CREATE INDEX IF NOT EXISTS idx_memory_user_type ON handler_memory(user_id, memory_type);
CREATE INDEX IF NOT EXISTS idx_memory_user_active ON handler_memory(user_id, is_active, importance DESC);
CREATE INDEX IF NOT EXISTS idx_memory_user_recent ON handler_memory(user_id, last_reinforced_at DESC);

ALTER TABLE handler_memory ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users own memories" ON handler_memory;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_extraction_log_source ON handler_memory_extraction_log(user_id, source_type, source_id);
ALTER TABLE handler_memory_extraction_log ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users own extraction log" ON handler_memory_extraction_log;
...
CREATE INDEX IF NOT EXISTS idx_device_schedule_user ON device_schedule(user_id, status, scheduled_at);
ALTER TABLE device_schedule ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users own device schedules" ON device_schedule;
...
CREATE INDEX IF NOT EXISTS idx_device_events_user ON device_events(user_id, created_at DESC);
ALTER TABLE device_events ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users own device events" ON device_events;
...
DROP POLICY IF EXISTS "Users own dm poll state" ON dm_poll_state;
