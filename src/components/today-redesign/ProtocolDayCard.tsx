...
export function ProtocolDayCard() {
  ...
  const modeColor = (m: string | null): string => {
    ...
    if (m === 'shutting_down' || m === 'dissociating') return '#e6bd80';
  ...
  return (
    <div style={{
      background: milestone ? 'linear-gradient(92deg, #2a1f0a 0%, #1f1608 100%)' : '#171017',
      border: `1px solid ${milestone ? '#e6bd80' : '#4a2438'}`,
      borderRadius: 10, padding: 14, marginBottom: 16,
    }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 8 }}>
        <div style={{ fontSize: 32, fontWeight: 800, color: milestone ? '#e6bd80' : '#f2e9e6', fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>
          Day {days}
        </div>
        <div style={{ fontSize: 11, color: '#9c8590', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
          of {(phase || 'phase_1').replace('_', ' ')}
        </div>
      </div>

      {milestone && (
        <div style={{ fontSize: 11, color: '#e6bd80', marginBottom: 8, fontStyle: 'italic' }}>
          Milestone day. Irreversibility compounds.
        </div>
      )}

      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, fontSize: 10.5 }}>
        <Tag label="denial" value={`day ${denialDay ?? 0}`} color="#edaec5" />
