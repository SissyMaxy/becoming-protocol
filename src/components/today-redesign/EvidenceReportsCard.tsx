  ...
  return (
    <div style={{ background: '#171017', border: '1px solid #4a2438', borderRadius: 10, padding: 14, marginBottom: 16 }}>
      ...
      {reports.map(r => {
        ...
        return (
          <div key={r.id} style={{
            background: '#0f0a0e', border: '1px solid #2b1d29', borderRadius: 7, padding: 10, marginBottom: 6,
          }}>
            <button
              onClick={() => setOpenId(isOpen ? null : r.id)}
              style={{
                width: '100%', textAlign: 'left', background: 'none', border: 'none', color: '#f2e9e6',
                padding: 0, cursor: 'pointer', fontFamily: 'inherit',
                display: 'flex', alignItems: 'center', gap: 8,
              }}
            >
              <span style={{ fontSize: 11.5, fontWeight: 700, color: '#edaec5' }}>week of {weekLabel}</span>
              <span style={{ fontSize: 10.5, color: '#9c8590' }}>
                {isOpen ? '↑ collapse' : '↓ read'}
              </span>
            </button>
            {isOpen && (
              <div style={{
                marginTop: 8, fontSize: 11.5, color: '#d5c3ca', lineHeight: 1.6,
                whiteSpace: 'pre-wrap', background: '#0a0709', padding: 10, borderRadius: 5,
