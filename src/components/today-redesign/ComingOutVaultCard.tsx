...
export function ComingOutVaultCard() {
  ...
  const riskColor = (n: number) => n >= 8 ? '#f47272' : n >= 6 ? '#e6bd80' : n >= 4 ? '#edaec5' : '#6ee7b7';

  return (
    <div style={{ background: '#171017', border: '1px solid #7a1f4d', borderRadius: 10, padding: 14, marginBottom: 16 }}>
      ...
      <div style={{ fontSize: 10.5, color: '#9c8590', marginBottom: 10, fontStyle: 'italic' }}>
      ...
      {letters.map(l => {
        ...
        return (
          <div key={l.id} style={{
            background: '#0f0a0e',
            border: `1px solid ${isSent ? '#2b1d29' : '#4a2438'}`,
            borderRadius: 8, padding: 11, marginBottom: 8,
            opacity: isSent ? 0.75 : 1,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: '#f2e9e6' }}>{l.recipient_name}</span>
              <span style={{ fontSize: 9.5, color: '#9c8590' }}>· {l.recipient_relationship}</span>
              ...
              <span style={{ fontSize: 9.5, color: '#edaec5', marginLeft: 'auto' }}>{l.channel} · {l.tone}</span>
            </div>

            {!isOpen ? (
              <button onClick={() => setOpenId(l.id)} style={{
                width: '100%', textAlign: 'left', padding: '6px 8px', borderRadius: 5, border: '1px solid #2b1d29',
                background: '#0a0709', color: '#9c8590', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit',
              }}>
                {current.slice(0, 90)}… <span style={{ color: '#edaec5', fontWeight: 600 }}>read</span>
              </button>
            ) : (
              <>
                <div
                  onClick={() => copyBody(l)}
                  style={{
                    fontSize: 12, color: '#f2e9e6', lineHeight: 1.55, padding: 10, borderRadius: 6,
                ...
                {l.disclosure_scope.length > 0 && (
                  <div style={{ fontSize: 10, color: '#9c8590', marginBottom: 6 }}>
                    discloses: <span style={{ color: '#edaec5' }}>{l.disclosure_scope.join(', ')}</span>
                  </div>
                )}
                ...
                {!isSent && responseMode !== l.id && (
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    ...
                    {l.status !== 'ready' && (
                      <button onClick={() => markReady(l)} style={btnStyle('#edaec5')}>Mark ready</button>
                    )}
                    <button onClick={() => markSent(l)} style={btnStyle('#c9557f', true)}>I sent it</button>
                    <button onClick={() => withdraw(l)} style={btnStyle('#9c8590', false, true)}>Withdraw</button>
                    <button onClick={() => setOpenId(null)} style={btnStyle('#9c8590', false, true)}>Collapse</button>
                  </div>
                )}

                {isSent && !l.response_reaction && responseMode !== l.id && (
                  <button onClick={() => setResponseMode(l.id)} style={btnStyle('#edaec5')}>Log response</button>
                )}
                {l.response_reaction && (
                  <div style={{ fontSize: 10.5, color: '#d5c3ca', padding: '5px 8px', background: 'rgba(110,231,183,0.08)', borderRadius: 4 }}>
                  ...
                  <div style={{ marginTop: 6 }}>
                    <input
                      ...
                      style={{
                        ...
                        padding: '5px 8px', fontSize: 11, color: '#f2e9e6', fontFamily: 'inherit', marginBottom: 5,
                      }}
                    />
...
function EditToggle({ l, edits, setEdits, saveEdit }: { l: Letter; edits: Record<string, string>; setEdits: (e: Record<string, string>) => void; saveEdit: (l: Letter) => void }) {
  ...
  if (!editing) {
    return <button onClick={() => { setEditing(true); setEdits({ ...edits, [l.id]: l.edited_body || l.body }); }} style={btnStyle('#edaec5', false, true)}>Edit</button>;
  }
  return (
    <div style={{ width: '100%', marginTop: 5 }}>
      <textarea
        ...
        style={{
          ...
          background: '#0a0709', border: '1px solid #7a1f4d', color: '#f2e9e6', fontFamily: 'inherit', marginBottom: 5,
        }}
      />
      <div style={{ display: 'flex', gap: 5 }}>
        <button onClick={() => { saveEdit(l); setEditing(false); }} style={btnStyle('#edaec5', true)}>Save edit</button>
        <button onClick={() => setEditing(false)} style={btnStyle('#9c8590', false, true)}>Cancel</button>
      </div>
    </div>
  );
}
...
function btnStyle(color: string, filled = false, outline = false): React.CSSProperties {
  return {
    ...
    border: outline ? '1px solid #2b1d29' : 'none',
...
function reactionColor(r: string): string {
  ...
  if (r === 'mixed') return '#e6bd80';
  if (r === 'no_response') return '#9c8590';
  return '#edaec5';
}
