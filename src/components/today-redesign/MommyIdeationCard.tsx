...
export function MommyIdeationCard() {
  ...
  const tone = !summary.last_run_at
    ? { bg: 'linear-gradient(135deg, #1a0f0f 0%, #150a0a 100%)', border: '#7a3f3f', accent: '#f0a0a0', label: 'NO DATA' }
    : !someOk
  ...
  const ProviderDot = ({ p }: { p: Provider }) => {
    const st = ps[p];
    const ok = st?.ok === true;
    const color = !st ? '#46333f' : ok ? '#8fd9b0' : '#f87171';
  ...
  return (
    <div style={{
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          {features.map((f, i) => (
            <div key={i} style={{ fontSize: 11, color: '#edaec5', padding: '3px 0', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ color: '#6d5a63', fontSize: 10 }}>{i + 1}.</span>
              <span style={{ flex: 1 }}>{f.title}</span>
              {f.category && (
                <span style={{ fontSize: 9, color: f.category === 'infra' ? '#8fd9b0' : '#fbb6ce', background: f.category === 'infra' ? '#0a1a0e' : '#1a0a14', padding: '1px 5px', borderRadius: 3 }}>
                  {f.category === 'infra' ? 'infra' : 'kink'}
                </span>
              )}
              {f.effort && (
                <span style={{ fontSize: 9, color: '#7a7480', border: '1px solid #4a2438', padding: '0 4px', borderRadius: 3 }}>{f.effort}</span>
              )}
      ...
      {ps.judge_summary && (
        <div style={{ fontSize: 10.5, color: '#9c8590', marginTop: 8, fontStyle: 'italic', lineHeight: 1.4 }}>
          {ps.judge_summary}
        </div>
      )}
    </div>
