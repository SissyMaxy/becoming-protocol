...
const MODES: Array<{ key: NonNullable<CaptureMode>; label: string; icon: string; hint: string; accent: string }> = [
  ...
  { key: 'progress_photo', label: 'Progress photo', icon: '🪞', hint: 'Underwear only. Front / side / back. Handler notes silhouette delta.', accent: '#edaec5' },
  { key: 'mirror_check', label: 'Mirror check', icon: '✨', hint: 'Mirror selfie. Handler reads posture, presentation, expression.', accent: '#6ee7b7' },
  { key: 'gina_text', label: 'Gina text screenshot', icon: '💬', hint: 'Screenshot of conversation with Gina. Extracts her quotes + reaction.', accent: '#e6bd80' },
];
  ...
  return (
    <div style={{ background: '#171017', border: '1px solid #4a2438', borderRadius: 10, padding: 14, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        ...
        {mode && (
          <button onClick={() => setMode(null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#9c8590', fontSize: 11, cursor: 'pointer', padding: 0 }}>
            ← back
          </button>
        )}
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 6 }}>
            {MODES.map(m => (
              <button key={m.key} onClick={() => setMode(m.key)} style={{
                background: '#0f0a0e', border: `1px solid ${m.accent}33`, borderRadius: 8,
                ...
                <div style={{ fontSize: 10, color: '#9c8590', lineHeight: 1.4 }}>{m.hint}</div>
              </button>
            ))}
          </div>
          <div style={{ fontSize: 10, color: '#7f6b74', marginTop: 8, textAlign: 'center' }}>
            all captures route through Claude vision · analysis feeds Handler context
          </div>
        </>
      ) : (
        <div>
          <div style={{ fontSize: 11.5, color: '#edaec5', fontWeight: 600, marginBottom: 8 }}>
