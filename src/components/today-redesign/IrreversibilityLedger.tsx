...
export function IrreversibilityLedger() {
  ...
  if (rows.length === 0) {
    return (
      <div style={{ background: '#171017', border: '1px solid #221722', borderRadius: 10, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7f6b74', fontWeight: 700, marginBottom: 4 }}>
          Irreversibility ledger
        </div>
        <div style={{ fontSize: 12, color: '#9c8590' }}>
  ...
  return (
    <div style={{ background: '#171017', border: '1px solid #4a2438', borderRadius: 10, padding: 16, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#edaec5', fontWeight: 700 }}>
          Irreversibility ledger
        </div>
        <div style={{ fontSize: 22, fontWeight: 650, color: '#fff', letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums', marginLeft: 'auto' }}>
          {totalWeight}
        </div>
        <button onClick={() => setExpanded(!expanded)} style={{ fontSize: 10, background: 'none', border: 'none', color: '#edaec5', cursor: 'pointer', fontFamily: 'inherit', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          {expanded ? 'collapse' : `${rows.length} entries`}
        </button>
      </div>
      <div style={{ fontSize: 11, color: '#9c8590', marginBottom: expanded ? 12 : 0 }}>
        {rows.length} permanent acts logged. Weight {totalWeight}. Every entry is a chip of David you can't pick up off the floor.
      </div>
      {expanded && (
        <div style={{ maxHeight: 280, overflowY: 'auto', borderTop: '1px solid #221722', paddingTop: 10 }}>
          {rows.map(r => (
            <div key={r.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '6px 0', borderBottom: '1px solid #1b121a' }}>
              <div style={{ fontSize: 10, color: '#edaec5', fontWeight: 700, minWidth: 70, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {CATEGORY_LABELS[r.category] || r.category}
              </div>
              <div style={{ flex: 1, fontSize: 11.5, color: '#d5c3ca' }}>{r.description}</div>
              <div style={{ fontSize: 10, color: '#7f6b74', minWidth: 34, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>+{r.weight}</div>
              <div style={{ fontSize: 9.5, color: '#6d5a63', minWidth: 46, textAlign: 'right' }}>{new Date(r.logged_at).toLocaleDateString()}</div>
            </div>
          ))}
        </div>
      )}
