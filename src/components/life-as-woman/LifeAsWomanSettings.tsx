  ...
  useEffect(() => {
    ...
    loadSettings(userId).then(s => {
      ...
      setSettings(s ?? {
        ...
        external_content_ingestion_enabled: false,
        external_content_allowed_sources: [],
      })
      setLoading(false)
    })
  ...
  return (
    <div style={{ background: 'var(--protocol-surface)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
      ...
      <div style={{
        ...
        />
        <ToggleRow
          label="Curated external content ingestion"
          sublabel="OAuth or user-provided links only. Approved items feed session packs, never infinite scroll."
          on={!!settings.external_content_ingestion_enabled}
          onChange={v => patch({
            external_content_ingestion_enabled: v,
            external_content_allowed_sources: v ? ['reddit', 'direct_link'] : [],
          })}
        />
      </div>
    </div>
