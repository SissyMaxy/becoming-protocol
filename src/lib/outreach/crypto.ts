// AES-256-GCM token encryption for outreach OAuth credentials.
//
// Same scheme as src/lib/calendar/crypto.ts — separate keys per integration
// so a leak of one doesn't compromise the other. Stored format:
//   base64( iv(12) || ciphertext_with_tag )
//
// Key seam: OUTREACH_TOKEN_KEY is a 32-byte secret, base64-encoded.
...
async function importKey(keyB64: string): Promise<CryptoKey> {
  ...
  if (raw.length !== KEY_BYTES) {
    throw new Error(
      `OUTREACH_TOKEN_KEY must decode to ${KEY_BYTES} bytes (got ${raw.length})`,
    );
  }
