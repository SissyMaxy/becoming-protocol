/**
 * Dommy Mommy in-fantasy distortion / contradiction layer.
 *
 * When `gaslight_intensity` is non-off, recall/tease surfaces route the
 * source quote through `distortQuote()` before sending it to the LLM
 * (and before user-visible composition). The result is a deliberately
 * inaccurate "memory" — tense shifted, severity escalated, count
 * inflated, etc. Every distortion is logged so the meta-frame reveal
 * can show truth-vs-distorted.
 *
 * Determinism: every transformer takes a seed, so the same (quote, seed)
 * always produces the same output. Tests assert against fixed seeds.
 *
 * Hard rules — these CANNOT be relaxed by intensity:
 *   - `applyDistortion('off', ...)` always returns the original quote
 *      with `type: null` and `applied: false`.
 *   - `SAFETY_KEYWORDS` short-circuit distortion. If the quote talks
 *      about billing/login/safeword/medical/legal/financial, no
 *      distortion is applied — Mama doesn't gaslight on top of safety
 *      surfaces.
 *
 * This module is duplicated at supabase/functions/_shared/distortion.ts
 * for edge-function imports (Deno can't reach src/). Keep them in sync.
 */
