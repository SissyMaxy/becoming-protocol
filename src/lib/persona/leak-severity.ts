// Voice-leak severity classifier — deterministic mirror of
// classify_voice_leak_severity() in migration 301. Same text → same
// severity, always. Edit both this file AND the SQL function together;
// the supabase/functions/_shared/leak-severity.ts edge-function copy
// must also stay in lockstep.
...
// Map severity to a stable category bias. The cascade only ever pulls
// from the existing arousal_touch_tasks vocabulary — no new categories.
...
// Expiry window per severity. High severity = longer "no relief" window.
...
// Deterministic fallback if the LLM refuses or errors. Firm/disappointed
// per the spec: never abusive, never body-shaming, never medical, never
// raging. Each one is severity-appropriate.
