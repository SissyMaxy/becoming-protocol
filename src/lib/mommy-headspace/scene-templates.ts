/**
 * Scene authoring templates — browser/Node mirror of
 * supabase/functions/_shared/scene-templates.ts. Keep in sync.
 */
...
const COFFEE_SHOP_VOICE: SceneTemplate = {
  ...
  intensity_band: 'firm',
  build_preparation: (ctx) => ({
    wardrobe: [pickOwned(ctx, 'jewelry', 'the small studs')],
    bring: [],
...
const ERRAND_LIP_GLOSS: SceneTemplate = {
  ...
  build_preparation: (ctx) => ({
    wardrobe: [],
    bring: [pickOwned(ctx, 'makeup_product', 'a clear gloss')],
...
export function planWeek(opts: WeekPlanContext): PlannedScene[] {
  ...
  const dayOffsets = [1, 3, 5].slice(0, count);
  const hours = [10, 17, 21];
