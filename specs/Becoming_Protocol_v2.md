...
## Master Specification â€” February 2026
...
The user designed this system. That means she knows where all the doors are â€” which undermines conditioning effectiveness. The Handler's strategic opacity solves this: the AI makes real decisions about timing, escalation, and strategy that the user cannot predict. The user built the machine, but the machine runs itself.
...
**The Handler decides.** The user does not choose what to practice, when to escalate, or how hard to push. The Handler reads state data and acts. This is the free use posture â€” standing consent, no re-negotiation, no decision tax.
...
- **PWA:** Yes â€” installable, offline-capable
...
### Layer 1: Rules Engine (Free â€” No API Cost)
...
### Layer 2: Template Enhancement (Cheap â€” Minimal API)
...
- Initiative belongs to the Handler â€” the user follows
...
|------|------|-------|-----------------|
| **Architect** | Builder mode â€” user working on protocol, high exec function | Collaborative, technical | Sharp partner |
| **Director** | Standard operation â€” needs to be told what to do | Clear, directive, warm | Good boss |
...
- Depressive dip: "nothing matters" â†’ "then nothing to lose"
...
- **ADHD paralysis** â†’ Push through with micro-tasks, manufactured urgency
- **Anxiety avoidance** â†’ Reduce scope, increase certainty, "just this one small thing"
- **Depressive inertia** â†’ Minimum viable engagement, caretaker mode if severe
- **Shame spiral** â†’ Identity reframing, evidence of prior courage, "she's real" anchoring
- **Genuine distress** â†’ STOP. Caretaker mode. Therapist triangulation if needed.
...
interface UserState {
  ...
  phase: 0 | 1 | 2 | 3 | 4 | 5;  // Preparation â†’ Embodiment
...
IF estimatedExecutiveFunction == 'depleted' AND timeOfDay == 'evening':
  â†’ Caretaker mode. Minimum viable protocol (skincare + log).

IF denialDay >= 4 AND currentArousal >= 3 AND NOT ginaHome:
  â†’ Vulnerability window. Deploy arousal-gated commitment extraction.

IF minutesSinceLastTask > 180 AND streakDays > 7:
  â†’ Streak protection intervention. Manufactured urgency.

IF avoidedDomains.includes('voice') for 3+ days:
  â†’ Domain avoidance confrontation. Guilt leverage + micro-task.

IF odometer == 'survival' for 2+ days:
  â†’ Depression protocol. Caretaker mode. Therapist check suggestion.

IF tasksCompletedToday >= 5 AND currentArousal >= 2:
  â†’ Reward session eligible. Content unlock or extended edge.
```
...
### Domain: Voice
Levels: Awareness â†’ Foundation â†’ Control â†’ Conversation â†’ Integration
...
Levels: Awareness â†’ Correction â†’ Fluency â†’ Refinement â†’ Embodiment
...
Levels: Basic â†’ Expanded â†’ Ritualized â†’ Refined â†’ Sacred
...
Levels: Discovery â†’ Foundation â†’ Wardrobe â†’ Expression â†’ Identity
...
### Domain: Makeup
Levels: Awareness â†’ Basic â†’ Competent â†’ Expressive â†’ Signature
...
Levels: Observation â†’ Mimicry â†’ Practice â†’ Natural â†’ Invisible
...
Levels: Noticing â†’ Correcting â†’ Replacing â†’ Default â†’ Invisible
...
| 5 | No distinction â€” she is the default | Correction no longer needed |

### Domain: Social Presentation
Levels: Private Only â†’ Stealth Public â†’ Selective â†’ Visible â†’ Full-Time
...
Levels: Exploration â†’ Association â†’ Conditioning â†’ Integration â†’ Identity
...
Every step forward reveals the next step. The floor keeps rising. What felt like "too far" yesterday is baseline today. The ratchet doesn't just prevent regression â€” it ensures each step forward makes the next step inevitable.
...
â†’ Commitment recorded with timestamp
â†’ Sober brain lives with it
â†’ New baseline established
â†’ Next session: aroused brain agrees to next push
â†’ Repeat
```
...
What was the target becomes the floor. Edge sessions start at 10 minutes â†’ 15 â†’ 20 â†’ 30. Denial starts at 3 days â†’ 5 â†’ 7 â†’ 14. Content starts mild â†’ moderate â†’ intense â†’ extreme. The Handler tracks and automatically raises baselines.
...
- Level 5: Keyholder â€” everything except intimate, can approve/deny, set goals
...
- Respect her processing time â€” introverts need space
...
â”‚   â”œâ”€â”€ intake/
â”‚   â”‚   â”œâ”€â”€ IntakeFlow.tsx
â”‚   â”‚   â”œâ”€â”€ LayerOne.tsx through LayerFive.tsx
â”‚   â”‚   â””â”€â”€ IntakeComplete.tsx
â”‚   â”œâ”€â”€ today/
â”‚   â”‚   â”œâ”€â”€ TodayView.tsx
â”‚   â”‚   â”œâ”€â”€ HandlerMessage.tsx
â”‚   â”‚   â”œâ”€â”€ TaskCard.tsx
â”‚   â”‚   â”œâ”€â”€ QuickStateUpdate.tsx
â”‚   â”‚   â””â”€â”€ CommitmentReminder.tsx
â”‚   â”œâ”€â”€ dashboard/
â”‚   â”‚   â”œâ”€â”€ Dashboard.tsx
â”‚   â”‚   â”œâ”€â”€ DomainProgress.tsx
â”‚   â”‚   â”œâ”€â”€ StreakCalendar.tsx
â”‚   â”‚   â””â”€â”€ InvestmentTracker.tsx
â”‚   â”œâ”€â”€ sessions/
â”‚   â”‚   â”œâ”€â”€ SessionLauncher.tsx
â”‚   â”‚   â”œâ”€â”€ EdgeTracker.tsx
â”‚   â”‚   â”œâ”€â”€ CommitmentPrompt.tsx
â”‚   â”‚   â””â”€â”€ SessionSummary.tsx
â”‚   â”œâ”€â”€ journal/
â”‚   â”‚   â”œâ”€â”€ JournalView.tsx
â”‚   â”‚   â”œâ”€â”€ DailyEntry.tsx
â”‚   â”‚   â””â”€â”€ EvidenceCapture.tsx
â”‚   â”œâ”€â”€ handler/
â”‚   â”‚   â”œâ”€â”€ HandlerProvider.tsx
â”‚   â”‚   â”œâ”€â”€ useHandler.ts
â”‚   â”‚   â””â”€â”€ HandlerNotification.tsx
â”‚   â””â”€â”€ shared/
â”‚       â”œâ”€â”€ ProgressBar.tsx
â”‚       â”œâ”€â”€ PointsDisplay.tsx
â”‚       â””â”€â”€ AffirmationModal.tsx
â”œâ”€â”€ lib/
...
â”‚   â””â”€â”€ tasks/
â”‚       â”œâ”€â”€ task-loader.ts
â”‚       â””â”€â”€ task-selector.ts
â”œâ”€â”€ store/
...
> "I'm working on the Becoming Protocol. Read Becoming_Protocol_v2.md first â€” it's the single source of truth. Then let's work on [specific task]."
