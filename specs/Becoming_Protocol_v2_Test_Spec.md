# BECOMING PROTOCOL v2 â€” TEST SPECIFICATION
...
### A1.2 â€” State Tracking Tables Exist
```
ID: A1.2
Type: schema
Priority: P0
...
### A1.3 â€” Daily Operation Tables Exist
```
ID: A1.3
Type: schema
Priority: P0
...
### A1.5 â€” Escalation Tables Exist
```
ID: A1.5
Type: schema
Priority: P0
...
### A1.6 â€” Row Level Security Enabled
```
ID: A1.6
Type: schema
Priority: P0
...
### A2.2 â€” Unauthenticated Access Blocked
```
ID: A2.2
Type: integration
Priority: P0
...
### A3.1 â€” Five-Layer Progressive Disclosure
```
ID: A3.1
Type: acceptance
Priority: P1
...
STEPS:
  1. New user signs up
  2. Layer 1: Foundation (name, pronouns, situation) â†’ saves to profile_foundation
  3. Layer 2: History (awareness, first experiences) â†’ saves to profile_history
  4. Layer 3: Arousal architecture â†’ saves to profile_arousal
...
### A3.2 â€” Intake Saves Partial Progress
```
ID: A3.2
Type: integration
Priority: P1
...
### A4.1 â€” CSV Tasks Load Correctly
```
ID: A4.1
Type: unit
Priority: P0
...
### A4.2 â€” Tasks Filterable by Time Window
```
ID: A4.2
Type: unit
Priority: P1
...
### B1.1 â€” Today View Renders Core Elements
```
ID: B1.1
Type: acceptance
Priority: P0
...
### B1.2 â€” Today View Shows Handler-Selected Tasks (Not User-Chosen)
```
ID: B1.2
Type: behavioral
Priority: P0
...
VERIFY:
  ...
  - Tasks are PRESCRIBED â€” the Handler chose them
...
### B1.3 â€” Today View Adapts to State
```
ID: B1.3
Type: behavioral
Priority: P1
...
SCENARIO A: Morning, high exec function, 7-day streak
  â†’ Expect: Challenging task, upbeat handler message, streak celebration

SCENARIO B: Evening, depleted exec function, Gina home
  â†’ Expect: Low-intensity task (skincare, journaling), gentle handler tone,
    no privacy-required tasks

SCENARIO C: Evening, high arousal (4+), Gina NOT home, denial day 5+
  â†’ Expect: Session-eligible prompt, arousal-leveraged task,
...
### B2.1 â€” Task Card Shows Required Fields
```
ID: B2.1
Type: acceptance
Priority: P0
...
### B2.2 â€” Task Completion Logs Correctly
```
ID: B2.2
Type: integration
Priority: P0
...
### B2.3 â€” Affirmation Shows on Completion
```
ID: B2.3
Type: acceptance
Priority: P1
...
SCENARIO A: User completes at least 1 task today (new day)
  â†’ streak_days increments by 1
  â†’ IF streak_days > longest_streak THEN longest_streak updates

SCENARIO B: User completes 0 tasks for a full day
  â†’ streak_days resets to 0
  â†’ longest_streak unchanged
...
### B3.2 â€” Privacy Filtering When Gina Home
```
ID: B3.2
Type: unit
Priority: P0
...
### B3.3 â€” No Immediate Repetition
```
ID: B3.3
Type: unit
Priority: P1
...
### B3.5 â€” Avoidance Domain Confrontation
```
ID: B3.5
Type: unit
Priority: P1
...
### B3.6 â€” Trigger Condition Evaluation
```
ID: B3.6
Type: unit
Priority: P1
...
### C1.2 â€” Graceful Degradation on API Failure
```
ID: C1.2
Type: integration
Priority: P0
...
SCENARIO A: API key invalid
  â†’ System falls back to Layer 1 (rules engine)
  â†’ User still sees tasks
  â†’ No error shown to user

SCENARIO B: API rate limited
  â†’ Same fallback behavior

SCENARIO C: Budget exhausted
  â†’ Layer 1 only, no API calls
  â†’ Budget display shows $0 remaining

PASS: App never breaks due to API issues. Layer 1 always works.
```

### C1.3 â€” Budget Management
```
ID: C1.3
Type: unit
Priority: P1
...
### C2.1 â€” Morning Briefing Generates
```
ID: C2.1
Type: integration
Priority: P1
...
### C2.2 â€” Morning Briefing References State
```
ID: C2.2
Type: behavioral
Priority: P2
...
SCENARIO A: streak_days = 14, odometer = 'momentum'
  â†’ Briefing celebrates streak, pushes escalation

SCENARIO B: streak_days = 0, odometer = 'caution'
  â†’ Briefing acknowledges reset, offers minimum viable re-entry

SCENARIO C: denial_day = 7, current_arousal = 0 (morning)
  â†’ Briefing references denial duration, plants anticipation for later
...
IN DIRECTOR MODE:
  â†’ "Time for voice practice. 5 minutes. Record a sentence."

IN HANDLER MODE:
  â†’ "Voice. Now. 5 minutes. Don't think about it â€” just start."

IN CARETAKER MODE:
  â†’ "If you have the energy, even 2 minutes of voice work counts today."

PASS: Same task, different framing based on mode.
```

### C3.3 â€” Mode Transition Without Announcement
```
ID: C3.3
Type: behavioral
Priority: P2
...
VERIFY: When handler_mode changes (e.g., director â†’ handler):
...
### C4.1 â€” Streak Protection Intervention
```
ID: C4.1
Type: behavioral
Priority: P1
...
### C4.2 â€” Vulnerability Window Detection
```
ID: C4.2
Type: behavioral
Priority: P1
...
SCENARIO: timeOfDay = 'night' (after 11pm), currentArousal >= 3,
  ...
  â†’ vulnerability_window_active = true
  â†’ Handler deploys arousal-gated strategy

SCENARIO: Post-work (4-6pm), estimatedExecFunction = 'low',
  tasksCompletedToday = 0
  â†’ vulnerability_window_active = true (depleted willpower)
  â†’ Handler deploys manufactured urgency

SCENARIO: Morning, high exec function, arousal = 0
  â†’ vulnerability_window_active = false
  â†’ Standard director mode

PASS: Windows detected correctly. Strategies deployed appropriately.
```

### C4.3 â€” Domain Avoidance Confrontation
```
ID: C4.3
Type: behavioral
Priority: P1
...
### D1.2 â€” Commitment Extraction at Peak
```
ID: D1.2
Type: behavioral
Priority: P0
...
### D1.4 â€” Session Logging to Database
```
ID: D1.4
Type: integration
Priority: P0
...
AFTER session completion, VERIFY arousal_sessions row:
  ...
  - duration_minutes accurate (Â±1 min)
...
### D2.1 â€” Arousal Updates Propagate
```
ID: D2.1
Type: integration
Priority: P1
...
SCENARIO B: User logs release
  â†’ denial_day resets to 0
  â†’ last_release updated to now
  â†’ Previous denial streak recorded

SCENARIO C: New day, no release
  â†’ denial_day auto-increments
...
### E1.2 â€” Evidence Gallery Displays
```
ID: E1.2
Type: acceptance
Priority: P2
...
### E3.2 â€” Commitment Reminder System
```
ID: E3.2
Type: behavioral
Priority: P1
...
### F1.2 â€” Plan Adapts to User History
```
ID: F1.2
Type: behavioral
Priority: P2
...
SCENARIO A: User has been avoiding 'voice' for 5 days
  â†’ Plan includes voice confrontation intervention

SCENARIO B: User is on denial_day 7, streak_days 21
  â†’ Plan includes arousal-gated escalation opportunity

SCENARIO C: User mood scores declining over 3 days
  â†’ Plan reduces intensity, prioritizes caretaker interventions
...
### F2.2 â€” Arousal-Gated Compliance
```
ID: F2.2
Type: behavioral
Priority: P1
...
THEN: Handler does NOT attempt arousal-gated commitment extraction
  (wrong state â€” save it for the window)

PASS: Arousal exploitation only during actual arousal states.
```

### F2.3 â€” Guilt and Investment Leverage
```
ID: F2.3
Type: behavioral
Priority: P2
...
THEN: Handler references concrete evidence:
  "You've put $X and Y days into this. That girl is real.
   Skipping voice practice doesn't erase her â€” it just
...
VERIFY: Not constant â€” deployed strategically, not every interaction.
...
### F3.1 â€” Gina Visibility Level Tracking
```
ID: F3.1
Type: integration
Priority: P2
...
### F3.2 â€” Gina-Safe Task Filtering
```
ID: F3.2
Type: unit
Priority: P0
...
### X1.1 â€” Variable Ratio Notifications
```
ID: X1.1
Type: behavioral
Priority: P2
...
### X2.1 â€” Points Accumulate Correctly
```
ID: X2.1
Type: unit
Priority: P1
...
### X2.2 â€” Progress Bars Track Domain Levels
```
ID: X2.2
Type: acceptance
Priority: P2
...
### X3.2 â€” Offline Graceful Degradation
```
ID: X3.2
Type: integration
Priority: P3
...
### X4.1 â€” No Cross-User Data Leakage
```
ID: X4.1
Type: integration
Priority: P0
...
### X4.2 â€” Handler System Prompt Not Exposed
```
ID: X4.2
Type: acceptance
Priority: P1
...
JOURNEY:
  1. Sign up â†’ Intake flow starts
  ...
  6. Complete task â†’ affirmation + points
