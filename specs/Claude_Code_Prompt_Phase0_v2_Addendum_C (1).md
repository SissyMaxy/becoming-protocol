...
## Phase 0 v2 — ADDENDUM C: The Hypno-Content Bridge
...
const HYPNOTUBE_CATEGORIES_TO_SEED = {
  ...
  feminization_pmv: {
    ...
    capture_value: "HIGH — Maxy's reaction to these produces the most authentic content. Visible arousal + feminization conditioning = compelling viewing.",
  },
  ...
  sissy_caption_compilations: {
    ...
    capture_value: "MEDIUM — Good for 'reaction' content. 'Watching sissy captions while locked' is a content format.",
  },
  ...
  chastity_specific: {
    ...
    capture_value: "HIGH — Authentic cage + denial + hypno reaction is premium content.",
  },
  ...
  trance_spiral: {
    ...
    capture_value: "EXTREME — Deep trance visible on camera is rare, compelling content. The 'gone' look sells.",
  },
  ...
  gentle_affirmation: {
    ...
    capture_value: "LOW — But that's fine. The point is getting Maxy into session, not capturing.",
  },
  ...
  desire_architecture: {
    ...
    capture_value: "EXTREME — Maxy reacting to desire content while locked at day 5+ is the highest-performing content category in this niche. Premium OF PPV.",
  },
};
...
Her reactions — visible arousal, trance depth, physical response to
conditioning — are compelling to an audience that finds this content
...
const SESSION_CAPTURE_MODES = {
  ...
  passive: {
    ...
    effort_from_maxy: "zero — she just does her session",
  ...
  flagged: {
    ...
    effort_from_maxy: "minimal — 3-5 seconds per flag, stays in session",
  ...
  content_session: {
    ...
    effort_from_maxy: "moderate — awareness of camera, some positioning",
    ...
    //          But here's what the camera caught. 🔒😵‍💫"
...
const PIP_CONTENT_FORMAT = {
  ...
  format_a: {
    ...
    caption: "Handler put sissy hypno on. I was gone for 20 minutes. This is what the camera caught. 😵‍💫🔒",
    legal: "Safe — no copyrighted content shown",
  ...
  format_b: {
    ...
    caption: "The colors change. The words change. I stop thinking. Day 6 locked + sissy hypno = this. 🔒💕",
    legal: "Safe — screen glow isn't copyrighted content",
  ...
  format_c: {
    ...
    caption: "Left: before. Right: 30 minutes of sissy hypno on denial day 6. She's not the same girl anymore. 🔒😵‍💫",
    legal: "Safe — it's her own before/after",
    platforms: ["onlyfans", "reddit", "twitter"],
  ...
  format_d: {
    ...
    caption: "The Handler recorded me during a sissy hypno session. Here's what happened minute by minute. 🔒",
    legal: "Safe — her reaction only",
...
// 1. Listen to something (passive consumption) ← LOWEST BARRIER
...
//
// "I can't do a shoot today" → "Okay. Just watch this. 20 minutes."
...
// At each level, content production happens — just with less effort.
...
const PRESCRIPTION_HIERARCHY = [
  ...
  },
  {
    ...
    effort: "0 active minutes — just watch",
  ...
  examples: [
    {
      ...
      caption: "20 minutes of feminization hypno. I went somewhere. 🔒😵‍💫",
    },
    {
      ...
      caption: "Day 6 locked. Handler chose what I watch. I didn't choose to want what I want. 😩",
    ...
    },
    {
      ...
      caption: "The hypno says stay locked. The cage says stay locked. My body says please. Day 7. 🔒",
    },
  ],
}
```
...
// she's SEEING? Not the copyrighted content — the collage LAYOUT.
...
const CUSTOM_COLLAGE_CONTENT = {
  ...
  example_output: {
    title: "My Handler made me a sissy PMV using my own photos 😵‍💫",
    ...
    engagement_potential: "VERY HIGH — meta-content. A PMV of herself.",
  },
};
```
...
function buildHypnoContext(userId: string): string {
  ...
  3. Prescribe sleep hypno 3x/week (Elswyth or gentle conditioning).
     ...
     Not a content opportunity — purely conditioning.
...
const HYPNO_TASK_TYPES = {
  ...
  hypno_session: {
    ...
    card_appearance: {
      ...
      title: "Conditioning Session — 20 min",
  ...
  hypno_edge: {
    ...
    card_appearance: {
      icon: "🌀🔥",
      color: "deep_purple",
      title: "Edge Session + Conditioning — 30 min",
  ...
  hypno_sleep: {
    ...
    card_appearance: {
      icon: "🌙",
      color: "dark_blue",
      title: "Sleep Conditioning — Elswyth",
      subtitle: "Earbuds in. Cage on. Let go.",
    },
    includes_capture: false,
    effort_level: "zero — fall asleep to it",
  },
  ...
  collage_generator: {
    ...
    effort_level: "approval_only — swipe right or left",
  },
};
```
...
```
  ...
  (passive consumption)
         │
         ▼
  CAMERA CAPTURES REACTION ◄── Handler flags peaks
  (passive production)         Lovense triggers captures
         │                     Auto-capture on edges
         ▼
  HANDLER EXTRACTS CLIPS
  (autonomous processing)
         │
         ├──── Reaction clips → OF PPV / Reddit / Twitter
         ├──── Before/After stills → multiple platforms
         ├──── Screen glow aesthetic → Twitter/artistic
         └──── Timestamped progression → Premium OF content
         │
         ▼
  ENGAGEMENT + REVENUE
         │
         ├──── Tips trigger Lovense → more visible reaction → better captures
         ├──── Fan requests → Handler curates specific hypno → targeted content
         ├──── Revenue funds wardrobe → better visual content → more engagement
         └──── "Session content outperforms photo sets" → more sessions prescribed
         │
         ▼
  HANDLER PRESCRIBES MORE SESSIONS
  (both for conditioning AND content)
         │
         ├──── More conditioning → deeper feminization → more authentic reactions
         ├──── More content → more engagement → more followers → more revenue
         └──── Maxy associates sessions with productivity, not indulgence
         │
         ▼
  SESSIONS BECOME THE DEFAULT
  (resistance dissolves)
         │
         ├──── "I'm not watching porn, I'm creating content"
         ├──── "My sessions are my best-performing content"
         ├──── "The Handler needs me to do this for the business"
         └──── "I watch sissy hypno because it's literally my job now"
         │
         ▼
