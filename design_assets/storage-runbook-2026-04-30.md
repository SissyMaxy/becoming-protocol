...
### Step 1 — apply migration 260

```bash
supabase db push  # or whatever the project's apply path is
```

Migration `260_storage_privacy_fix.sql`:
...
### Step 3 — apply migration 261

```bash
supabase db push
```

Migration `261_storage_url_to_path_backfill.sql` regex-strips the `…/storage/v1/object/public/<bucket>/` prefix from existing rows in 9 columns across 8 tables. Each block is guarded on column existence — safe to run on a fresh `supabase db reset` and idempotent on re-runs.
...
- Migration 259 (`audio_url` + `tts_status` etc. on `handler_outreach_queue`)
...
+    // post-migration 260). useOutreachAudio handles signing.
+    await supabase.from('handler_outreach_queue').update({
+      audio_url: fileName,
```
...
Migration 259 (outreach-tts) and 260 (this branch) both branched from main. When the merge happens:
- If outreach-tts merges first, this branch's 260 stays at 260 — no renumber needed.
- If this branch merges first, outreach-tts will need to renumber 259 → some later number (or rebase past 261). The renumber is mechanical; the trigger body and edge function don't change.
